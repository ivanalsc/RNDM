...
export default function MusicPage() {
  ...
  const loadEntries = async () => {
    try {
      setLoading(true)
      const data = await getMediaEntries("music")
      setEntries(data)
    } catch (error) {
    ...
    <div className="flex min-h-screen flex-col bg-[#f8f5f2]">
      ...
      <main className="flex-1">
        <div className="container py-6">
          <h1 className="text-3xl font-serif font-medium mb-6">Music</h1>
          <div className="space-y-6">
            {loading ? (
              <div className="text-center py-8">Loading...</div>
            ) : entries.length === 0 ? (
              <div className="text-center py-8 text-gray-500">No music shared yet</div>
            ) : (
              entries.map((entry) => (
